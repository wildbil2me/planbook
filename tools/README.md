...
| `wo-sweep.mjs` | The verifier's 45-check standing sweep as greps — the checks a `grep` settles correctly, with their allowlists written down, including the three active `no-cache` stanzas in `_headers`, the backup nag's collection list against `docs/data-model.md`, and both copies of the repo-write guard — plus, since WO-2.48, the list of guarded scripts itself, derived and diffed against what § 15 declares, and since 2026-08-20 the drawings in `design/mockups/` — § 19, which is the machine-run half of `design/mockups/PROTOCOL.md` — and since 2026-08-30 the two files the pipeline is typed and dispatched from, § 21, which reaches `.claude/commands/wo.md` and `.claude/agents/work-order-orchestrator.md` **by naming their paths** because `.claude` is in `IGNORE_DIRS` and stays there — and, since WO-1.41 the same day, `AGENTS.md` against `CLAUDE.md` as § 21's **second pair**, on four claims about the rules both files carry, which needs no path trick (the walk already reaches the repository root) and is named all the same, because a pair is a hand-maintained list rather than a directory scan — and, since WO-1.42 on 2026-08-31, the count at the head of this very row, § 22, which is held against the number of results the run emits and against no figure written down inside the tool — and, since WO-5.7 on 2026-09-12, § 24, the second door out of an outreach draft: `src/outreach.js` has no writer in it at all and `copyDraft()` in its neighbour reaches none, which is § 17's division of labour over a file whose neighbour legitimately DOES write, plus the one claim the browser harness said it could not make, that nothing asynchronous sits between that function's gate and its `writeText(` — the clipboard API is refused outside a user gesture, the harness has to grant the permission in order to press the control at all, and a granted permission is exactly what stops the activation rule applying — and, since WO-1.45 on 2026-09-24, § 25, which reads `verify-shell.mjs` for the **shape** of WO-1.44's section containment: both section lists handed to `runSection()`, no `.run(` anywhere outside its body, the one inside it awaited between a `try` and a `catch` that records a `check(…, false, …)` with no `return` or `throw` in front of it. **It is a tripwire on the text and not a proof of the behaviour, and a green § 25 does not mean the containment works:** it cannot see whether a throw actually reaches that catch (a section's un-awaited promise or a CDP callback escapes it at runtime), what the FAIL records (a wrong count or a record in a dead branch passes), whether `recoverPage()` tells the truth, an aliased `run` call, or the exit code. Only a `verify-shell.mjs` run with a throw planted in a section proves those, and whoever edits `runSection()`, `recoverPage()` or the browser loop owes that run again — see *A section that throws* under § "Driving a browser over CDP" — and, since WO-1.52 on 2026-09-24, § 26, which holds every **status claim in a planning document** — the `.html` directly in `plans/`, plus `data-viewer.html` here, **reached by path**, with `IGNORE_DIRS` and `--audit`'s walk both untouched — against what `wo-gate.mjs --list` says `parseFile()` read out of the tracker, which is the **reference half** and is never learned the other way. It reports a contradicted claim **in a live voice** as a `REVIEW`, not a `FAIL`, because a planning document is allowed to be historical: an `<em>(…)</em>` note, a `class="was"` passage, a sentence dated `until YYYY-MM-DD`, a document on its dated list (`plans/return-brief.html`, by its own words) and a past-tense, denied, hypothetical or other-clause claim are all excused, and **silence is the green state**. It reads a claim two ways: **as a sentence**, an id followed by a status before the next id, and **as a card**, an id cell and a later status cell of one `div`/`span`/table row, however much prose — full stops and all — sits in the cells between (added by WO-1.52's first correction round, after `plans/wo-3-18-runbook.html`'s dependency strip read as zero claims because a description cell ended the sentence before the status). Its mechanical half is a second check that does fail, and it asks **per document** that at least one claim was read, so a named document yielding nothing goes red by name rather than passing beside one that yielded plenty; `data-viewer.html` states no status and is excused by its own words. `--claims-in=<file>` points it at a file out of git history instead — `git show 06bfa06:plans/wo-3-18-video-runbook.html` into a scratch file reports the four statements `a16b87c` repaired, and the same at `a16b87c` is clean. **It cannot see itself:** it reads none of this row, its own banner, WO-1.52 or `CLAUDE.md`, every one of which quotes a false status on purpose to explain it. `node tools/wo-sweep.mjs` |
...
| `wo-cost.mjs` | What each dispatch cost, from the session transcripts. `node tools/wo-cost.mjs` — and, since WO-1.39 on 2026-09-24, **what the usage window has spent**: `--window` sums the last five hours across **every** directory under `~/.claude/projects/` (the window is the account's, and a project-scoped reading under-reads) in a **proxy** unit — output ×5, cache-write ×1.25, input ×1, cache-read ×0.1, the unit `plans/session-limits.md` defines and measured its reference points in — and prints it beside those points: the median dispatch (6.0M) and the death cluster (16.4M p25, 20.3M median). **It is calibration, not a ceiling**, and the line says *proxy* where the number is. It counts every transcript line carrying usage, duplicates included, because that is what the calibration counted — a deduplicated sum reads WO-5.5 at 3.6M where the table says 6.8, and would under-read by half against its own reference points. A missing, unreadable or empty root, or a root that is one project's directory, **exits 1 and prints no figure**; a genuinely quiet window is said in words, and no path prints `0.0M`. `--swapped` records an account swap in `wo-cost-swap.json` **beside** the root (so `~/.claude/`, never this repository), and the sum starts there instead of five hours ago until the marker ages out by itself. `--root <dir>` moves the whole thing, marker included, which is the seam `wo-gate.mjs --start --window-root` and `--self-check` use. `wo-gate.mjs --start` prints its one-line form, `--line`, and **clears on any figure and on none** — advisory, never refusing. `node tools/wo-cost.mjs --window` |
...
| `data-viewer.html` | **Not a script** — a page, opened in a browser. A read-only viewer for one year document: drop a backup `.json` on it anywhere, or, served from the app's own origin, read that origin's IndexedDB. A collapsible tree with every opaque id annotated with the name it belongs to, a substring search over keys and values, and the raw JSON of whatever is selected. It **interprets nothing** — no count is recomputed, no rule is checked, an absent key is drawn as absent — which is the line between it and the checking tool booked as [WO-1.50](../plans/work-orders/phase-1-shell-store-roster.md#wo-150--a-document-you-can-read-is-not-a-document-anything-checks). `supports`, `guardians` and a student's own contact fields are masked until asked for (`CLAUDE.md` § Accommodations rule 1 has no `tools/` carve-out). It never writes: the store is opened `readonly`, and where `indexedDB.databases()` is unavailable it deletes the empty shell `open()` makes rather than leaving a database for the app to find. Serving it from the app's origin works because WO-8.12 narrowed `sw.js`'s navigate branch to `APP_DOCUMENT` — before that, every path at that origin was answered with the cached shell. |
| `inspector-mockup.html` | **Not a script, and not built** — the drawing behind [WO-1.50](../plans/work-orders/phase-1-shell-store-roster.md#wo-150--a-document-you-can-read-is-not-a-document-anything-checks): a *checking* panel over the same document the viewer reads, findings in four families, each naming the mechanism and offering its paths. Static, invented data, and it deliberately does **not** follow `design/mockups/PROTOCOL.md` — the reason is argued at the top of the file, and § 19 does not reach `tools/`. Parked 2026-09-06 in favour of the viewer above; the work order's Traps are the half a drawing cannot carry. |
...
***That count was hand-maintained and nothing checked it until 2026-08-31.*** *It read `33` from
WO-5.1's § 20 on 2026-08-28 until WO-1.40 corrected it on 2026-08-30, which is one section's worth of
rot in two days — and WO-1.41 moved it from `36` to `38` later that same day, two checks for one new
pair in § 21, by the same hand and with the same nothing watching. A reader's only recourse was the
`N checks ·` line the run itself prints, and a* `wo-sweep.mjs` *§ 21 that had been asked to also count
its own siblings would have been a second subject in a work order that had one.* **WO-1.42 ended
that.** *§ 22 reads the number out of the row above and holds it against `results.length` — what this
run actually pushed — so a drifted count is now a red run rather than something a reader has to
happen to notice. It is the last section in the file out of necessity, and the check beside it
asserts exactly that, because a census reading the total from anywhere earlier reports its own
position instead.* **The one edit, when the sweep gains or loses a check, is the number in the row
above**; *there is no second figure inside the tool to keep in step with it. Two things it still does
not do: the recorded number describes a healthy tree, so a run already red elsewhere can emit a
different number and be red here as a consequence rather than a cause — read the other failures
first; and it watches that* **one** *number and not every figure on this page.* `wo-sweep.mjs` *§ 11
checks the call-site count recorded for* `verify-shell.mjs`*, and everything else here is still typed
by a person with nothing reading it.*
...
plants forty-three violations against them, runs the script over the copy, and deletes the directory on
...
**WO-1.29 added two on 2026-09-07**, about what a found field is allowed to **contain** where
WO-1.27's four are about where it is written: a `**Owes**` field holding prose with no work-order id
...
**WO-1.39 added three on 2026-09-24**, the first here to run a **second script** and the first to write
something that is not a tracker: a transcript tree in the sandbox, handed to `--start` through
`--window-root`. `--start` prints the window line and still claims at **25.0M**, past the death
cluster, summing two project directories and refusing a line seven hours old that sits in a freshly
written file; a window it cannot read — no `wo-cost.mjs`, a root that does not exist, a root with no
transcript — reads `window  unavailable — <reason>` and still claims, while `--window` on its own
exits 1, and neither prints `0.0M`; and `--swapped` restarts the count, writes its marker beside the
root it was given, and stops mattering five hours later. `wo-cost.mjs` is copied into the sandbox for
those three and removed after each, so every earlier plant's `--start` reads the quiet *unavailable*
path. Proved against the pre-WO-1.39 script with `--against` (3 red of 43) and by nine mutations of
scratch copies of `tools/`, each reddening the plant named after it. **No plant reads the real
`~/.claude/projects`**, deliberately — the figures are asserted to the decimal — and none asks
whether 6.0M and 20.3M are still true.
`43 plants, 43 caught, 0 missed` / `PASS | 43 of 43 plants were caught`, read
...
| **the whole of `tools/wo-gate.mjs` as it stood before WO-1.29** — `--against` over `git show HEAD:tools/wo-gate.mjs`, so nothing in the tree was mutated at all | **1 red**: the zero-id plant, on all ten of its assertions at once, and two of them are the defect verbatim — *"--tick exited 0 on a `**Owes**` field that names no work order"*, which then wrote `ROADMAP.md`, the phase file and `README.md`; and *"the `**Owes**` section printed rows for 3 work order(s) and its tally says 4"*. The positive control beside it stays **green**, which is the shape to expect and the reason it is a pair: a control never reddens against the absence of the thing it guards |
...
**WO-5.2 moved it from 1182 to 1208**: twenty-six literal call sites, the whole of
`verify/templates.mjs` — a file that work order created — none of them inside a loop and none a
failure arm, so the section contributes twenty-six executed results to a green run. Its two guards
are `skip()` calls again (no `window.planbook`, and a fixture that did not install), on the WO-4.5
and WO-5.1 pattern directly above, which is why they move this number by nothing. **The run prints
1223**: `1223 checks · 1223 passed · 0 failed · 0 skipped`, 36,525 lines, 29.9 lines per check, 413s, exit 0, measured
2026-08-28 on the delivered tree. The gap between sites and results stays at −15. *(That section is
also the fourth `byHand` in `verify/touch-targets.mjs`'s `VIEW_PLAN`, and the first one that is
...
**WO-6.8 moved it from 1364 to 1378, and the executed count from 1373 to 1387 — fourteen sites,
fourteen results.** All fourteen are literal call sites in a new block at the foot of
`verify/glance-quiet.mjs`'s fixture branch, *"the glance page: today and this week, waiting to be
graded, closing in (WO-6.8)"*, standing on that section's own fixture guard; none is inside a loop —
the one loop in the block walks the days of the lead time and checks once, after it — and none is a
failure arm, so the gap between sites and results stays at −16. **Three pre-existing checks in the
same file changed what they assert, and none moves the count**: the quiet chips now read *Nothing
scheduled through …*; the grades-due probe asserts the owner's 2026-09-16 ruling (never a week item)
where it asserted the opposite; and two checks that read *"this row draws nothing in its place"* now
...
**A section that throws no longer kills the run, and it is still loud** (WO-1.44). Every section is
...
**`wo-sweep.mjs` § 25 fences the shape of that containment and not its behaviour**: it goes red if a
`.run(` appears outside `runSection()`, if either section list stops being handed to it, if the
`.run(` inside it stops being awaited or leaves the `try`, or if the catch stops recording a `false`,
returns before recording it, or rethrows. It cannot see whether a
throw reaches the catch at runtime, whether the recorded count is right, or whether `recoverPage()`
is honest. **A green § 25 is not evidence the containment works** — the planted-throw run described
under WO-1.44's count history above is, and anyone who edits `runSection()`, `recoverPage()` or the
browser loop owes that run again.
