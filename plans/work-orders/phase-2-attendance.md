...
**Acceptance**
- [x] Today 2026-08-19, one class, terms `Quarter 1` 2026-09-02 – 2026-10-31: the strip opens with
      ...
      *(Divergence, found by driving it 2026-08-19: the last clause is true in PORTRAIT and cannot be
      true in landscape. The strip walks weekdays BACK from the day it is built from, so a six-column
      window ending on 9/2 necessarily draws Aug 26 – Sep 1 behind it — greyed `Off term`, nothing
      tappable, no button — which is **decision 3's soft wall rather than a leak**, and it is what the
      👤 line below is asking about when it says "portrait draws one column and it is 9/2". So the
      harness asserts the newest column, today's absence and the band on the landscape grid, and takes
      a second reading at 834×1112 where one column is drawn and it is 9/2. The check that claimed the
      literal sentence was red on a correct app.)*
- [x] **9/2 is live with nothing pressed** — a cell tap marks it and the record lands on `2026-09-02` —
      and **9/3 is not**: it carries a ✏️, and it accepts marks only after that ✏️ is pressed.
- [x] `◀ Earlier` from 9/2 still reaches August, and every column out there is greyed `Off term` with
      nothing tappable in it. The soft wall, proved from the inside out.
- [x] `Later ▶` reaches 10/31 and is disabled there saying why, **on a document with an empty
      calendar** — so the stop being proved is the term's end and not a day off.
- [x] **A class with no dated terms behaves exactly as it does today**: the anchor is today, no future
      column carries a ✏️, there is no bar, and `writableDate()` refuses tomorrow.
- [x] **The selected term never bounds a write.** With Q1 ending Oct 31 and Q2 starting Nov 3, the Q1
...
- [x] Today Nov 4, preference on Q1: **arriving at the screen selects Q2** and the strip opens on today.
...
- [x] **Nothing moves the term while the screen is open**: with Q2 selected and the screen painted,
      repeated repaints leave the preference unchanged; only an arrival moves it.
- [x] Terms labelled `Trimester 1` and `Trimester 2` produce the same sentences with those labels. No
      quarter vocabulary anywhere in the output.
- [x] `grep -rn "editPastDay|lockPastDay|editingPast|futureLimit" src/ tools/ TESTING.md` returns
      nothing — the rename swept rather than shadowed.
- [x] `node tools/verify-shell.mjs` green with its check count recorded, and **one mutation proof over
...
- [x] 👤 Pre-mark 9/2 on the iPad, then read the term percentage: the day counts as a meeting from that
...
So does the landscape reading the verifier raised as a design question rather than a defect: asked
whether Aug 26 – Sep 1 greyed behind 9/2 reads right against the owner's own *only show dates in
the selected term*, the answer was that it does. The anchor stays the NEWEST column and the window
goes on walking back from it — decision 3's soft wall, settled on the glass rather than from the
Deliverables.*
