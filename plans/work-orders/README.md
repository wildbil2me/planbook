...
| 44 | [WO-5.9](phase-5-outreach.md#wo-59--the-hitless-draft-is-written-but-never-driven) The hitless draft is written but never driven | S | — | 🎒 `tools/verify/contact-log.mjs` — **Nothing blocks it.** An hour, whenever that file is open — rows 40 and 41 both touch the send flow and either would carry it. Booked **2026-08-29** by WO-5.4's own verifier, which found it and declined to close it inside the row that made it. `recordHandoff()` writes `ruleId: hit ? hit.ruleId : ''` and **the false branch is never driven**: both contacts the fixture writes are Ada's, who trips two rules. The *consequence* of an empty id is proved in `cooldown-quiet.mjs`, but off a **planted** record — so the reader is proved to tolerate absence and the writer is never proved to produce it. Two regressions pass today, in opposite directions: dropping the guard throws inside a click handler that calls no `preventDefault()`, so the mail app opens and nothing is logged; inventing an id silences a signal nobody wrote about, which is the mutation WO-5.4 caught, inverted |
| 46 | [WO-1.39](phase-1-shell-store-roster.md#wo-139--the-window-is-spent-by-measure-and-read-by-feel) The window is spent by measure and read by feel | M | — | After WO-1.38. `--start` asks whether the gates are clear and nothing asks whether there is a **window** to fit the dispatch in. **The `Size` column is not a capacity estimate**: S ran 6.8–13.3M weighted units and M ran 7.2–15.7M, overlapping almost entirely, and the priciest S outran five of six Ms — so a budget rule that fires on the big ones fires on the wrong ones. The data is already on disk at a **two-second lag** in the transcripts `wo-cost.mjs` reads. **Advisory, never refusing**, on row 36's precedent that only a person can pick. Three traps carry it: a project-scoped read under-reads (16 project dirs, 3 active in 24h), an account swap over-reads with nothing on disk recording it, and a `0.0M` meaning *no data* must not look like one meaning *fresh window* |
