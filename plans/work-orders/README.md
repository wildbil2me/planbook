...
|---|---|---|---|---|
| 1 — Shell, store, roster | 42 | 33 | — | 🔨 IN PROGRESS (reopened seventeen times; last on 2026-08-30) |
...
| 46 | [WO-1.40](phase-1-shell-store-roster.md#wo-140--the-file-a-human-types-is-outside-every-fence) The file a human types is outside every fence | S | — | **Whenever the tracker is quiet, and it is the fifth of the tracker rows** — 12, 14, 15, 16, 46. Booked **2026-08-30** by WO-1.38's verifier as a thing nobody had claimed. WO-1.38 changed the pipeline in **six** files and wrote five: `.claude/commands/wo.md` went on telling its caller to relay an Acceptance list from the one stop that now deliberately returns none, and the only material for satisfying it is the implementer's self-claims — **the WO-3.5 defect one level above the orchestrator, in the file with no agent between it and a human**. *The prose was corrected the same day, directly; this row is the fence, not the repair.* **`.claude/commands/` is named nowhere in this system** — no work order, no `--audit`, and `.claude` sits in `wo-sweep.mjs`'s `IGNORE_DIRS`, which is right for app code and is not a reason for no check at all. **Read its second trap before writing a line:** the two files are *supposed* to differ, so what is wanted is contradiction rather than symmetry, and `REVIEW` may be the honest state |
| 47 | [WO-1.41](phase-1-shell-store-roster.md#wo-141--the-two-files-that-must-never-drift-apart-are-held-together-by-nothing) The two files that must never drift apart are held together by nothing | M | — | **Whenever the tracker is quiet, and it is the sixth of the tracker rows** — 12, 14, 15, 16, 46, 47. Booked **2026-08-30** alongside row 46, and **deliberately booked before row 46 is built**: `CLAUDE.md` states *“a rule changed here is changed there in the same sitting”* in bold and **nothing enforces it** — a grep over `tools/*.mjs` finds two mentions of `AGENTS.md`, neither a check. It is row 46's defect on a surface an order of magnitude larger: 200 lines against 613, with the shared rules deliberately worded differently in each. **It depends on row 46 rather than duplicating it** — these two files sit at the repository root and the sweep already walks them, so the half that carries over is the comparison, and booking this first is what makes row 46 build one that takes a pair. **Read its fourth trap before writing a line:** the set of shared rules cannot be inferred from prose, and a hand-maintained list is the honest mechanism |
