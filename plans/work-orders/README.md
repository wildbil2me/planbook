...
| 5 — Outreach | 4 | 2 | — | 🔨 IN PROGRESS — WO-5.1 and WO-5.2 both ✅ DONE 2026-08-28, every line of each closed the same day; WO-5.3 and WO-5.4 ⬜ |
...
| 22 | [WO-5.3](phase-5-outreach.md#wo-53--send-flow) Send flow | M | — | **Next in this phase.** Row 21 ✅ 2026-08-28, so it is buildable today. `mailto:` only — a mail scope reads "Send email as you," and the architecture table forbids it. It reads `templatesFor(doc, tone, audience)` for the picker and the same `blocked` flag the preview's strip draws |
| 23 | [WO-5.4](phase-5-outreach.md#wo-54--contact-log--history) Contact log & history | M | — | After row 22 |
