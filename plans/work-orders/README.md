...
| 23 | [WO-5.4](phase-5-outreach.md#wo-54--contact-log--history) Contact log & history | M | — | After row 22. **Its dependency is 🔨 rather than ✅ and the two open lines are 👤 rather than 📆**, so the gate will refuse it until they are read — which is a person on an iPad rather than a build. It owns the `contact` entry in `log[]` outright, including the stale line in `src/merge-fields.js`'s header (~130) that still says WO-5.3 writes one |
