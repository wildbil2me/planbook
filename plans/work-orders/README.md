...
| 88 | [WO-8.17](phase-8-packaging.md#wo-817--an-open-app-only-looks-for-an-update-when-it-loads-a-page) An open app only looks for an update when it loads a page | S | — | **After row 87, owner-directed.** Booked **2026-09-26** from WO-7.7's hardware reading: v132 went live and neither device moved on its own, because nothing calls `registration.update()` and a browser looks only when a page loads. Checks on return to the screen and shows a notice outside About. **Two Open lines are the owner's before dispatch**: offer or reload by itself, and where the notice sits |
