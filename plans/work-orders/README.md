...
| 80 | [WO-8.15](phase-8-packaging.md#wo-815--the-homepage-google-is-given-is-an-empty-gradebook) The homepage Google is given is an empty gradebook | S | — | **Before row 9, and it is built.** Booked and built **2026-09-25** out of a fresh-eyes read of row 9's runbooks: the form's Homepage field pointed at the app, which a reviewer opening it cold sees as *No classes yet.* with no description and no visible privacy link. `about.html` is at the root and green at the desk; **It is deployed and `verify-deploy.mjs` is green at `/about` (`8b188bf`); what is left is one 👤 reading on the iPad and the laptop.** Row 9 depends on it, so row 9's gate report is red until it is ticked |
