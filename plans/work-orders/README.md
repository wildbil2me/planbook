...
| 87 | [WO-7.9](phase-7-sync.md#wo-79--a-fresh-device-cannot-open-the-year-it-already-has-in-google-drive) A fresh device cannot open the year it already has in Google Drive | M | — | **After row 85, which it depends on, owner-directed.** Booked **2026-09-26** after the owner connected a fresh laptop and found no way in: sync matches only its own `docId`, and first run makes a new one. The owner ruled that first run offers Drive **or** a backup. **Two Open lines are the owner's before dispatch.** Whether it goes ahead of row 9 is the owner's call: the demo video films a sync between two devices that are already set up, so it does not need this |
