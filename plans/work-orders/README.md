...
| [`phase-1-shell-store-roster.md`](phase-1-shell-store-roster.md) | WO-1.1 … WO-1.53 | Phase 1 |
...
|---|---|---|---|---|
| 1 — Shell, store, roster | 53 | 45 | — | 🔨 IN PROGRESS (reopened nineteen times; last on 2026-09-03) |
...
| 58 | [WO-1.52](phase-1-shell-store-roster.md#wo-152--a-planning-document-can-contradict-the-tracker-and-nothing-looks) A planning document can contradict the tracker and nothing looks | S | — | **Fifth row of this neighbourhood** — 12, 14, 15, 16, 57 and now this — and placed here rather than at the foot for row 57's reason: row 56 argues for sitting last and a row dropped beneath it falsifies the argument. Booked **2026-09-12** out of WO-1.31's verdict sitting, which found the instance and declined to widen a work order already verified. **Why it exists:** WO-1.31 made every `🔒` say what it waits for and `--audit` read them directory-wide — over `plans/work-orders/*.md` **and nothing else**. The hand-written HTML under `plans/` may state any work order's status, dependency or blocker and be wrong about it forever. **It had a live instance the day it was booked:** `plans/wo-3-18-video-runbook.html` carried **four** false statements — a `🔒 GATED` band five days after the lock came off, WO-7.2 as `⬜ not started` five days after it landed, a **stop**-styled blocker whose premise had been discharged, and three shots called *"the ones that do not exist yet"* — repaired by hand in `a16b87c`. **That repair is this row's reproduction**, which is more than WO-1.31 had. **Read the first Trap before writing a line:** a planning document is *allowed* to be historical, so the check tests for a contradicted claim in a live voice, not for staleness — lift WO-1.40's § 21 exclusions rather than re-deriving them, and **do not widen `IGNORE_DIRS` or `--audit`'s walk** or it drowns in `design/mockups/` and 371 dispatch files on the first run |
...
| 49 | [WO-1.43](phase-1-shell-store-roster.md#wo-143--a-broken-excuse-reads-as-a-contradiction-and-a-vanished-claim-reads-as-green) A broken excuse reads as a contradiction, and a vanished claim reads as green | S | — | 🎒 WO-1.42 — **an hour to fold into whichever sitting opens `wo-sweep.mjs` next**, and row 48 is the one already booked to. Nothing blocks it; WO-1.41 ✅ is its only dependency, and it is the eighth of the tracker rows — 12, 14, 15, 16, 46, 47, 48, 49. Booked **2026-08-30**, owner-directed, on WO-1.41's verifier's two proposals, taken as **one** row because they are two messages inside the same twenty lines of § 21. **Both of § 21's silences fail toward looking fine.** A `confinedTo` region that goes missing is reported as a contradiction with no hint the excuse mechanism is what broke — `lost` is computed at `tools/wo-sweep.mjs:2608` and consumed only in the green branch at `:2611`, so the branch a person acts on is the one that stays quiet. And `occurrences` is **one aggregate across every claim in a pair**, so a claim whose token is reworded out of the compared file stops testing anything behind a healthy-looking total; the reference side is already fenced by the paired FAIL check, which makes the compared side the only silent half and the half that gets rewritten most. **Read its first trap before writing a line:** a zero-occurrence claim must stay **green** — *absent is green* is the pair's whole premise — so this reports numbers and asserts nothing |
