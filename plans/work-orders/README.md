...
**Status vocabulary:** `⬜ NOT STARTED` · `🤖 CLAIMED — <dispatch>` · `🔍 AWAITING VERDICT — <dispatch>` ·
`🔨 IN PROGRESS` · `✅ DONE — <date>` · `🚧 BLOCKED` · `🔒 GATED` · `🚫 STRUCK — <date>` ·
`⏳ DEFERRED — <date>` · 🚩 marks a **go-live blocker**.
...
**The second half of that sentence is now a field rather than a hope, and the status line is where it
goes: `🔒 GATED — <what it waits for>`** (WO-1.31, 2026-09-12). The obligation had been on the
document and on nothing else since the word was defined, and a lock that names no gate **cannot be
re-checked and cannot expire** — [WO-7.2](phase-7-sync.md#wo-72--document-transfer--conflicts) kept
one for seventeen days after the argument behind it had been demolished for its whole phase, and it
was found by a human reading an unrelated runbook, not by any tool. `wo-gate.mjs` refuses a bare one
in the gate report and again directory-wide in `--audit`; the second is the one that matters, because
**a lock is the one status that guarantees nobody runs a gate report on it.** The grammar and its
limits are the **Status** row in § "Header fields" below — in particular that **nothing parses the
suffix**, and that `🚧 BLOCKED` is outside the rule. *(The check can only ask whether a gate is
**stated**, never whether it is still **live**. WO-3.18 is the reason to keep the difference in view:
its gate was stated, in its body, and had been discharged for five days when this landed — the lock
came off the same sitting, by hand, because that reading is a person's.)*
...
**`🔍 AWAITING VERDICT — <dispatch>` is the third of those facts, and 2026-08-30 (WO-1.38) is when the
pipeline started producing it on purpose.** The verifier is now dispatched from a **fresh session on
every work order** — ten dispatches have died at a handoff and six at that exact seam — so a row
spends real time **claimed, with nothing in flight and a finished implementer behind it.** That is
neither of the two above: `🤖` says a run is working on it and `🔨` says the work is half-done, and
this one says the work is **finished and unchecked**. `--handoff` writes it at the implementer's
return and `--tick` is the only way out; a verifier FAIL does not change it, because a correction goes
back to the same implementer and the verdict is still owed.

**`--release` refuses it, and that refusal is the whole point of the status.** From the running order
it looks exactly like the abandoned claim `--release` exists to clear — and releasing it would put a
complete, unverified tree back to `⬜ NOT STARTED`, where `next` offers it to somebody as unstarted
work and the result file describes a work order nobody is holding. The tool names the file it would
orphan. **The two have to be told apart by the tool rather than by whoever is reading**, which is why
this is a status and not a note in the `Suggested` column. It is **not `✅ DONE`**, so it satisfies no
dependency and a gate report still refuses it: built is not verified. **The glyph is the owner's to
change; the behaviour is not.**
...
| **Status**, specifically | One of the nine in the vocabulary above, and **four of them take a `— <suffix>`** — `✅ DONE — <date>`, `🚫 STRUCK — <date, reason>`, `⏳ DEFERRED — <date, reason>` and `🔒 GATED — <what it waits for>`, beside the two the tool writes for itself (`🤖 CLAIMED — <dispatch>`, `🔍 AWAITING VERDICT — <dispatch>`). **The first three carry a date and `🔒` carries a sentence**, which is the row worth reading twice: the paragraph above puts *what it is gated on* on the document, and until WO-1.31 nothing read it back. A lock that says nothing **cannot be re-checked and cannot expire** — WO-7.2 wore one for seventeen days after WO-3.10 had demolished the reasoning behind it for the whole of Phase 7, and what found it was a human reading an unrelated runbook. `wo-gate.mjs` now refuses a bare `🔒 GATED` twice: as a second problem under the refusal the lock already earns, and directory-wide in `--audit` — the half that matters, because **a lock is the one status that guarantees nobody runs a gate report on it**. **Nothing parses the suffix and nothing may start to**: it is prose for a person, and a reader here that expected a date would make the sentence unwriteable. Write the em dash, which is what all six spellings above use. **`🚧 BLOCKED` is deliberately outside this rule** — it has its own row in the vocabulary, and widening the check to every non-`⬜` status is how a small fence becomes a large one |
| **Depends on** | Every `WO-` token is a gate; anything else on the line is reported as prose for a human to read. A `…` between two `WO-` tokens is **two dependencies and not a range** — the gate warns about it and will never expand it, because WO-G2's `WO-2.5 … WO-2.7` left WO-2.6 gating nothing for a week and a parser guessing at ranges invents dependencies nobody typed. **A value with no `WO-` token in it must be one of this directory's ways of saying *no dependencies*** — `—`, `-`, `–`, `none`, `nothing`, or `nothing` with a reason after it — **and anything else is refused by the gate** (WO-1.30). Those six raise no problem and no note, and the report reads `depends nothing` for all of them; a clause that is none of them parses to **zero** dependencies, and zero dependencies is indistinguishable from every dependency satisfied. That is not hypothetical: WO-G4, the 1.0.0 call, read `every work order` and **cleared its own gate** with a note beside the `PASS`, and WO-8.1's `every phase` did the same until 2026-08-28. If a clause is what you mean, **name the work order it stands for and keep the clause after it** — the way WO-8.1's names WO-7.3 and WO-G4's names WO-8.1 — because ids and prose together are the ordinary case: the ids gate, the clause is printed for a human, and 20 work orders are written that way |
...
row 10 then, row 42 since it was re-placed on 2026-08-29, an hour of `index.html` — and [WO-6.5](phase-6-calendar-glance.md#wo-65--a-tapped-day-opens-on-that-day),
...
| [`phase-1-shell-store-roster.md`](phase-1-shell-store-roster.md) | WO-1.1 … WO-1.54 | Phase 1 |
...
| [`phase-5-outreach.md`](phase-5-outreach.md) | WO-5.1 … WO-5.16 | Phase 5 |
| [`phase-6-calendar-glance.md`](phase-6-calendar-glance.md) | WO-6.1 … WO-6.9 | Phase 6 |
| [`phase-7-sync.md`](phase-7-sync.md) | WO-7.1 … WO-7.9 | Phase 7 🔒 |
| [`phase-8-packaging.md`](phase-8-packaging.md) | WO-8.1 … WO-8.17 | Phase 8 |
...
reads them as prose about code, and a pair is a list a person maintains either way.)*
...
go red. The claim lists are in `tools/wo-sweep.mjs` § 21 — **two pairs, three claims and four** — and
they are meant to be amended by hand. **They are the fourth thing each pair has to keep in step**:
the two files, this table's row for them, and then the claims.
...
four about the rules `AGENTS.md` and `CLAUDE.md` both carry, and nothing else at all; the two rows
still reading *Nothing* are watched by no one.
...
| 5 — Outreach | 15 | 13 | 🚫 WO-5.11 | 🔨 IN PROGRESS — WO-5.1, WO-5.2 and WO-5.3 all ✅ DONE; the first two 2026-08-28 and the send flow **2026-08-29, when the owner read its last two 👤 lines on hardware** — the draft opened in the default client on both devices and the copy reached the sent folder. **That sitting is also where rows 5.5–5.8 came from**: six changes asked for after using it, booked rather than folded in, plus one defect fixed the same day (WO-5.2’s editor had no way out — see its note). **Three of those rows are already ✅ — WO-5.6, WO-5.5 and now WO-5.4, all 2026-08-29**; WO-5.7 and WO-5.8 ⬜. **WO-5.4 is the one that mattered for Sep 2**: it writes the `contact` entry two Phase 4 readers were already reading, so the cooldown has an input for the first time. **WO-5.9 joined them 2026-08-30** — a ride-along inside `tools/verify/contact-log.mjs` that drives the one branch of that writer nothing had ever walked, and the only row here that changed no app code. The phase stays 🔨 — the owner's call on the day, with two rows still ⬜. **WO-5.7 ✅ 2026-09-12**, its paste read on hardware the same day, and **WO-5.10 booked out of its verdict** — an XS ride-along on `src/outreach-view.js` for the one field the projector branch does not empty, found by the implementer and confirmed by the verifier — **and ✅ 2026-09-15**, its second Acceptance line reworded on the owner's ruling rather than closed by code. **WO-5.11 booked the same evening**, scheduled rather than 🎒: the owner registered Gmail as Chrome's `mailto:` handler and the PWA window navigated to Gmail's bare compose page — one attribute, and an iPad reading that decides it. **And 🚫 STRUCK before the evening was out** — the iPad passed and the laptop failed: with `_blank` Chrome left a blank tab on the `mailto:` URL and never reached the handler, so the attribute came out and the harness now asserts its absence. **And WO-5.8 was cut in four on 2026-09-20 before it was ever dispatched** — the last `Size L` row in the directory, cut along its own deliverable seams on the WO-6.4 precedent because one L is roughly a whole session window. It keeps its id, its running-order slot and the picker; [WO-5.13](phase-5-outreach.md#wo-513--every-template-whatever-the-recipient) took the audience-filter reversal and **must land first**, [WO-5.14](phase-5-outreach.md#wo-514--the-compose-doors-take-a-list-and-which-header-the-others-ride-in) took the address list and the disclosure ruling WO-5.8 said it owed, and [WO-5.15](phase-5-outreach.md#wo-515--one-contact-several-audiences) was booked out of the reading rather than out of WO-5.8's Deliverables — `writeContact({ audience })` is a scalar enum with three readers and a draft to two guardians and the counselor has no single value for it. **WO-5.13 and WO-5.14 both PASS their gates today**; WO-5.14 names WO-5.3 and not WO-5.12 on purpose, because WO-5.12 is 🔨 on an Outlook reading the owner has no account for. **Three of the four landed the same day they were cut — WO-5.13 ✅, WO-5.14 ✅ and WO-5.8 ✅, all 2026-09-20** — so the sentence above saying WO-5.8 is ⬜ is history rather than state, and the picker consumed both settled rulings instead of re-opening either. **The one judgment left to it was decided the other way from WO-5.3's**: an addressless recipient is now **genuinely unchoosable**, refused at the chip out loud, and the standing harness check that clicked her was rewritten rather than re-asserted — argued at `toggleOutreachRecipient()`, because under a selection the same tap means *also send this to her* over a message that is finished. **No 👤 and no 📆 on that row**, which is unusual enough to say. What is left of the cut is [WO-5.15](phase-5-outreach.md#wo-515--one-contact-several-audiences) ⬜, and the phase stays 🔨 on it and on WO-5.12 |
| 6 — Calendar & glance | 9 | 9 | — | 🔨 IN PROGRESS |
| 7 — Drive sync | 9 | 6 | — | 🔨 IN PROGRESS — WO-7.1 ✅ DONE 2026-08-24, all six lines closed the same day including the three that needed a human; WO-7.2 ✅ DONE 2026-09-07, both two-device lines closed by the owner on two Chrome profiles; WO-7.4 ✅ DONE 2026-09-26, the sign-in opened on the deployed domain and read on the laptop and the iPad; WO-7.5 ✅ DONE 2026-09-26, the header's sync button; WO-7.6 ✅ DONE 2026-09-26, the privacy documents say when Google's library loads, read off the deployed /privacy; WO-7.7 ✅ DONE 2026-09-26, a download repaints the open screen, read both ways on laptop and iPad; WO-7.3 still 🔒 |
| 8 — 1.0 packaging | 17 | 10 | — | 🔨 IN PROGRESS |
...
***And by hand again on 2026-09-12, from `51 | 43` to `52 | 43`.*** *Same cause, fifth instance, and
this one missed the* `--tick` *by minutes in the other direction:* [WO-1.52](phase-1-shell-store-roster.md#wo-152--a-planning-document-can-contradict-the-tracker-and-nothing-looks)
*was booked out of WO-1.31's verdict sitting, and WO-1.31's own tick had already run and already
written this table.* **The percentage went down again on a denominator that grew** *— 141 of 169 is
83%, where 141 of 168 was 84% — and the bar did not move, for the same reason as last time. Recomputed
against* `recomputeDashboard()`*'s rule, not by counting rows. The next* `--tick` *is still the
authority.*

***And Phase 5's by hand the same afternoon, from `9 | 8` to `10 | 8`.*** *Sixth instance:* [WO-5.10](phase-5-outreach.md#wo-510--the-status-line-is-the-one-field-the-projector-does-not-empty) *was booked out of WO-5.7's verdict sitting after WO-5.7's own tick had run. 142 of 170 rounds to 84% where 142 of 169 did, so the percentage held and the bar with it. Recomputed against* `recomputeDashboard()`*'s rule. The next* `--tick` *is still the authority.*

***And again that evening, from `10 | 8` to `11 | 8`.*** *Seventh instance:* [WO-5.11](phase-5-outreach.md#wo-511--a-web-mail-handler-takes-the-pwa-window-with-it) *was scheduled out of the same sitting, after the owner hit it on the laptop. 142 of 171 is 83% where 142 of 170 was 84%, the bar stays at eight. Recomputed against* `recomputeDashboard()`*'s rule. The next* `--tick` *is still the authority.*

***And back again before the evening was out, from `11 | 8 | —` to `10 | 8 | 🚫 WO-5.11`, and the total from `171 | 142 | 2` to `170 | 142 | 3`.*** *Eighth instance, and the first in the other direction:* [WO-5.11](phase-5-outreach.md#wo-511--a-web-mail-handler-takes-the-pwa-window-with-it) *was built, verified and* **struck on the laptop reading** *in one sitting — a struck row leaves the count, per the rule above, and takes its place in the* Not coming *column. 142 of 170 is 84% again. Recomputed by hand against* `recomputeDashboard()`*'s rule, because* `--tick` *cannot run on a struck row and nothing else rewrites this table. The next* `--tick` *is still the authority.*

***And forward once more the same night, from `10 | 8` to `11 | 8`, the total from `170` to `171`.*** *Ninth instance:* [WO-5.12](phase-5-outreach.md#wo-512--a-webmail-door-that-is-not-a-mailto) *was booked out of WO-5.11's striking — the answer to what struck it. 142 of 171 is 83% where 142 of 170 was 84%, the bar stays at eight, the* Not coming *column keeps its three. Recomputed against* `recomputeDashboard()`*'s rule. The next* `--tick` *is still the authority.*

***And Phase 6's by hand on 2026-09-15, from `6 | 4` to `8 | 4`, the total from `171 | 143` to `173 | 143`.*** *Tenth instance, and the first that booked two rows out of one:* [WO-6.7](phase-6-calendar-glance.md#wo-67--the-glance-pages-stack-its-readers-and-the-quiet-day) *and* [WO-6.8](phase-6-calendar-glance.md#wo-68--today-and-this-week-waiting-to-be-graded-and-closing-in) *were cut out of WO-6.4, which keeps its id and its roadmap box and drops L → M. 143 of 173 is 83% where 143 of 171 was 84%; the bar stays at eight. Recomputed against* `recomputeDashboard()`*'s rule. The next* `--tick` *is still the authority.*

***And Phase 4's by hand the same day, from `5 | 3` to `6 | 3`, the total from `173 | 144` to `174 | 144`.*** *Eleventh instance, and the first booked out of a **verdict** rather than a sitting:* [WO-4.6](phase-4-signals.md#wo-46--a-rule-that-cannot-fire-yet-is-a-different-sentence-from-a-rule-that-is-not-built) *came from WO-6.7's verifier, which upheld the owner's* `no` *on the quiet panel's "not yet" sentence and, in upholding it, named the reason as a hole in the engine rather than a choice about the screen —* `inertRules()` *filters on a static per-rule string and knows nothing about term length. **A declined sentence and a missing answer are two different findings**, and only the second is a work order; this is the second one, booked where the rules live rather than where the sentence was declined. 144 of 174 is 83% where 144 of 173 was also 83%; the bar does not move. Recomputed against* `recomputeDashboard()`*'s rule, and the next* `--tick` *is still the authority.*

***And Phase 6's by hand on 2026-09-16, from `8 | 5` to `9 | 5`, the total from `174 | 144` to `175 | 144`.*** *Twelfth instance, and the second booked out of a verdict:* [WO-6.9](phase-6-calendar-glance.md#wo-69--the-review-count-opens-a-page-that-shows-the-review) *came from WO-6.8's first verifier pass, which flagged that the review count's tap opens this month while its window can end in the next. The owner booked it rather than holding WO-6.8 open, because the fix is a choice between four landings and not a patch. 144 of 175 is 82% where 144 of 174 was 83%; the bar stays at eight. The next* `--tick` *is still the authority.*

***And Phase 1's by hand on 2026-09-17, from `52 | 43` to `53 | 43`, the total from `175 | 145` to `176 | 145`.*** *Thirteenth instance, and the third booked out of a verdict:* [WO-1.53](phase-1-shell-store-roster.md#wo-153--the-residue-meets-the-registers-earlier-page-for-a-week-every-september) *came from WO-6.9's verifier, which ran the harness on the real clock as well as on the shifted one and found the two disagreeing about a tree that had not moved — the WO-2.3 precondition reads the register's earlier page, and for a week every September that page holds the record `classes-terms.mjs` plants on a hard-coded date, with an eighth day where it is the dropped edge instead. Booked rather than folded into WO-6.9 because it is a harness defect the diff never touched, and rather than into WO-1.46 because that row's helper walks forward off a date the fixture is free to pick and this one is not. 145 of 176 is 82%, as before; the bar stays at eight. The next* `--tick` *is still the authority.*

***And Phase 1's by hand on 2026-09-24, from `53 | 48` to `54 | 48`, the total from `180 | 161` to `181 | 161`.*** *Fourteenth instance, booked out of WO-1.52's verdict minutes after its tick:* [WO-1.54](phase-1-shell-store-roster.md#wo-154--the-line-that-keeps-two-strips-apart-is-proved-by-nothing) *is the one line of § 26's new cell reader that survives its own deletion, booked as a 🎒 on* `tools/wo-sweep.mjs`*. 161 of 181 is 89%, as before; the bar stays at eight. Recomputed against* `recomputeDashboard()`*'s rule, not by counting rows. The next* `--tick` *is still the authority.*
...
***And Phase 8's by hand on 2026-09-21, from `13 | 7` to `14 | 7`, the total from `179 | 155` to
`180 | 155`.*** *Fifteenth instance, and the third booked out of one row rather than a sitting —
[WO-8.14](phase-8-packaging.md#wo-814--the-three-doc-links-in-about-are-measured-by-nothing) came out
of WO-8.13's landing the same day WO-8.13 was ticked, so this edit and that* `--tick` *fall in one
sitting and the tool wrote `155` before the hand wrote `180`.* **Order matters here and did not bite:**
`--tick` *recomputes from the phase rows it can see, so booking the new row* **after** *the tick is why
the two numbers agree — booking it first would have had the tool recompute over an* `⬜` *it had not
been told about, which is the same sitting reaching the same total by luck. 155 of 180 is 86% where
155 of 179 was 87%; the bar stays at eight. Recomputed against* `recomputeDashboard()`*'s rule. The
next* `--tick` *is still the authority.*

***And Phase 5's by hand on 2026-09-20, from `11 | 9` to `14 | 9`, the total from `176 | 150` to `179 | 150`.*** *Fourteenth instance, and the second that booked rows out of one row rather than out of a sitting or a verdict — the first was the tenth, Phase 6's, and this is the same move for the same reason.* [WO-5.8](phase-5-outreach.md#wo-58--several-recipients-and-one-of-them-is-primary) *was* **the last `Size L` in the directory** *and was cut in four before it was ever dispatched:* [WO-5.13](phase-5-outreach.md#wo-513--every-template-whatever-the-recipient)*,* [WO-5.14](phase-5-outreach.md#wo-514--the-compose-doors-take-a-list-and-which-header-the-others-ride-in) *and* [WO-5.15](phase-5-outreach.md#wo-515--one-contact-several-audiences) *came out of it, and WO-5.8* **keeps its id, its roadmap standing and its running-order slot** *and drops L → M — which is what keeps WO-8.13's 🎒 shelf resolving, since that cell names row 41 as the sitting that opens* `index.html`*. 150 of 179 is 84% where 150 of 176 was 85%; the bar stays at eight. Recomputed against* `recomputeDashboard()`*'s rule. The next* `--tick` *is still the authority.*

*(**Three of the four came out of the deliverables; the fourth came out of the reading.**
WO-5.13, WO-5.14 and what stays are WO-5.8's own Deliverables list, split where it already had
seams. [WO-5.15](phase-5-outreach.md#wo-515--one-contact-several-audiences) is not on that list and
never was — `writeContact({ audience })` is a scalar enum pinned in `docs/data-model.md` with three
readers, and a draft to two guardians **and** the counselor has no single value for it. **A cut is
also an audit**, which is the argument for doing it as its own sitting rather than discovering this
at hour four of an L. The owner's call the same day that it is booked rather than folded in,
because WO-5.8 lands writing `audienceOf(primary)` — **incomplete, not false**, since the message is
...
|---|---|---|---|---|
| 1 | [WO-4.1](phase-4-signals.md#wo-41--signal-engine--thresholds) **Signal engine & thresholds** | M | ✔ | ✅ **2026-08-19** — all five Acceptance lines, the day this table was written. Booked for **Aug 20–21, before the term**, and landed ahead of it. Unblocks five work orders — nothing else in Phase 4 or 5 can start |
...
| 5 | [WO-4.5](phase-4-signals.md#wo-45--cooldown--the-quiet-middle) Cooldown & the quiet middle | M | ✔ | 🔨 **2026-08-27, the first day of the fortnight it was booked into** — four of five boxes closed and both central claims mutation-proved; **only its 📆 real-data box is left and cannot close before ~Sep 16**. Three of the four 👤 readings closed on the iPad 2026-08-28; the fourth — the quiet middle at real length — is held open on purpose and re-read in the term (`TESTING.md` § WO-4.5) |
| 6 | [WO-G3](gates.md#wo-g3--ship-3-gate-signals) **Ship 3 gate: signals** | S | — | 🔒 **GATED 2026-08-28** — **~Oct 16**, once four weeks of real data exist. Marked when row 5 landed and left it the first `⬜` in the running order, so `next` was answering with a gate nobody can start; back to `⬜` when the data exists |
...
"startable now" since it was written, and row 11 was built on 2026-08-25, with Ship 3 still open.
...
*(**Row 11 was booked and built inside one day**, 2026-08-24 to 2026-08-25. A scheduling argument
sat here between those two commits and has been cut rather than kept: it reasoned at length about
which window WO-1.26 should take, against a tracker read that had WO-4.3 as `⬜ NOT STARTED` when it
was already built. **This directory keeps a superseded argument when it was once true, and this one
never was** — it was answered before it was written. What is worth carrying forward is only the
reading error: **`next` and a per-work-order gate report are a snapshot, and a parallel dispatch
moves the tracker under a long sitting.** Re-read the status immediately before scheduling against
it, never from a reading taken earlier in the same session.)*
...
| 80 | [WO-8.15](phase-8-packaging.md#wo-815--the-homepage-google-is-given-is-an-empty-gradebook) The homepage Google is given is an empty gradebook | S | — | **Before row 9, and ✅ 2026-09-25.** Booked and built **2026-09-25** out of a fresh-eyes read of row 9's runbooks: the form's Homepage field pointed at the app, which a reviewer opening it cold sees as *No classes yet.* with no description and no visible privacy link. `about.html` is at the root and green at the desk; **Deployed at `8b188bf`, `verify-deploy.mjs` green at `/about`, and read cold by the owner on the iPad and the laptop the same day.** Row 9 depended on it, and its gate report no longer names it |
| 82 | [WO-7.4](phase-7-sync.md#wo-74--the-deployed-app-has-no-sign-in-for-googles-reviewer-to-find) The deployed app has no sign-in for Google's reviewer to find | S | — | **Before row 9, which depends on it.** Booked **2026-09-25**, owner-directed, after row 80 closed: `hostAllowsSignIn()` shut the Drive section on the live domain, so the app the form names never used the scope it asks for. WO-7.3's flag deliverable, split off and moved ahead of the submission. Then row 9 |
| 83 | [WO-7.5](phase-7-sync.md#wo-75--the-header-says-how-fresh-this-devices-sync-is) The header says how fresh this device's sync is | M | — | **Before row 9, owner-directed.** Booked **2026-09-26** out of *"what about making the sync across devices more clean?"*: a header button, drawn only on a device that opted in, reading freshness rather than connection, and the tap that reconnects on the iPad inside the gesture. **All five of its questions ruled by the owner the same day**, four of them the amber questions on [`design/mockups/sync-button.html`](../../design/mockups/sync-button.html). Ahead of row 9 because the demo video films the sync flow, and filming it before this lands films a flow that is about to change |
| 84 | [WO-7.6](phase-7-sync.md#wo-76--the-privacy-documents-say-google-loads-only-on-the-connect-tap-and-since-wo-75-it-also-loads-at-launch) The privacy documents say Google loads only on the Connect tap | S | — | **Before the push that deploys row 83, owner-directed.** Booked **2026-09-26** out of WO-7.5's verdict: ruling 5 made an opted-in device load Google's library at every launch, and `privacy.html`, `docs/FERPA.md` and four notes still say it loads only on the Connect tap. Words only, never the behaviour. Row 83's 👤 lines need the deployed app, so this lands first and they ship together. Then row 9 *(**Overdue by construction, not by neglect**: row 83's own 👤 lines could only be read on the deployed app, so it was pushed before this landed, 2026-09-26. The live `/privacy` has been wrong since that push, and this is the next row to run.)* |
| 85 | [WO-7.7](phase-7-sync.md#wo-77--a-sync-that-downloads-leaves-the-screen-showing-the-document-it-replaced) A sync that downloads leaves the screen showing the document it replaced | S | — | **Before row 9, owner-reported.** Booked **2026-09-26** from the owner's iPad reading of row 83: a sync that pulls the newer copy down changes the data but not the open screen, because no screen subscribes to the store and `src/shell.js` has a comment saying they do. WO-7.2's defect, made easy to hit by row 83's header button. Before the demo video films a sync. Then row 9 |
| 86 | [WO-7.8](phase-7-sync.md#wo-78--the-stale-by-day-check-cannot-tell-a-calendar-day-from-24-hours-after-1512) The stale-by-day check cannot tell a calendar day from 24 hours after 15:12 | XS | — | **After row 85, owner-directed.** Booked **2026-09-26** from WO-7.5's verifier: both stale checks in `tools/verify/sync-button.mjs` pass under a 24-hour rule after mid-afternoon, so ruling 4 is only guarded for part of the day. Harness only: pin a fixed clock and plant syncs at 23:30 and 00:30 |
| 87 | [WO-7.9](phase-7-sync.md#wo-79--a-fresh-device-cannot-open-the-year-it-already-has-in-google-drive) A fresh device cannot open the year it already has in Google Drive | M | — | **After row 85, which it depends on, owner-directed.** Booked **2026-09-26** after the owner connected a fresh laptop and found no way in: sync matches only its own `docId`, and first run makes a new one. The owner ruled that first run offers Drive **or** a backup. **Ruling 1 taken the same day (leave the backup-then-Connect conflict). Ruling 2 the same day: variant B of [`design/mockups/first-run.html`](../../design/mockups/first-run.html), the doors inside the empty state. Nothing is open.** Whether it goes ahead of row 9 is the owner's call: the demo video films a sync between two devices that are already set up, so it does not need this |
| 88 | [WO-8.17](phase-8-packaging.md#wo-817--an-open-app-only-looks-for-an-update-when-it-loads-a-page) An open app only looks for an update when it loads a page | S | — | **After row 87, owner-directed.** Booked **2026-09-26** from WO-7.7's hardware reading: v132 went live and neither device moved on its own, because nothing calls `registration.update()` and a browser looks only when a page loads. Checks on return to the screen and shows a notice outside About. **Both rulings taken the same day**: offer the reload, never take it; a banner under the header, hidden in presentation mode. Nothing is open |
| 89 | [WO-5.16](phase-5-outreach.md#wo-516--a-restore-closes-the-outreach-draft-twice) A restore closes the outreach draft twice | XS | — | 🎒 `src/shell.js` — **Nothing blocks it.** Ten minutes inside any sitting that already has `src/shell.js` open. Booked **2026-09-26** from WO-7.7's verdict: `afterRestore()` calls `resetOutreach()` twice, both from WO-5.3's landing. Harmless, and WO-7.7's download repaint inherits it |
| 9 | [WO-3.18](phase-3-gradebook.md#wo-318--verification-submitted-) Verification submitted 🔒 | S | — | **Startable, and ⬜ rather than 🔒 since 2026-09-12.** Row 20 landed **2026-09-07**, which is the condition this work order's own lock note named — `src/drive-sync.js` carries the document both ways, so the scope is in use and the demo video has a file transfer to film. *(Read "**Gate-clear and not startable — behind row 20**" from 2026-08-28, and "**Startable now**" before that. The `🔒` outlived its condition by five days and was found by [WO-1.31](phase-1-shell-store-roster.md#wo-131--a--gated-work-order-that-never-says-what-it-is-gated-on)'s new check rather than by anyone reading this cell — which is that work order's whole argument, arriving live between its booking and its build.)* **Not an hour to fold into a teaching week**: a shoot, a console trip and a form, with the shot list in [`plans/wo-3-18-video-runbook.html`](../wo-3-18-video-runbook.html) |
| 10 | [WO-1.38](phase-1-shell-store-roster.md#wo-138--the-verifier-is-spawned-on-the-far-side-of-the-implementers-spend) The verifier is spawned on the far side of the implementer's spend | S | — | **Takes the slot WO-8.13 vacated on 2026-08-29** — placed here 2026-08-30 rather than at the foot, because a row whose whole value is *it protects every dispatch after it* is worth nothing dispatched last, and **position is not a fence** (row 36). Booked **2026-08-30** out of a transcript audit of all 124 dispatches: **ten carry a session-limit death and every one died at a handoff**, six of them at the implementer/verifier seam. The orchestrator is 15.3% of a dispatch's spend and 22% of its deaths — it is not dying of its own consumption. Makes the verifier's cold start **planned rather than imposed**, which the quota has done ten times and which WO-5.2 and WO-5.4 both proved survivable. **The deliverable is a third claim state**: a row whose implementer returned and whose verifier is owed reads today as an abandoned claim, and `--release` on it would orphan a finished tree. Owner's call the same day that the rule is **uniform, not size-gated** — see row 46 for why size predicts nothing **The discipline does not wait for the row**: taking the verifier as a second session and reading the window before `--start` can start on the very next dispatch, so this hardens a habit rather than gating one |
...
| 13 | [WO-1.28](phase-1-shell-store-roster.md#wo-128--a-dependency-waiting-on-the-calendar-blocks-work-that-is-ready-to-build) A calendar-bound dependency blocks buildable work | M | — | ✅ **2026-08-26** — all nine Acceptance lines, including the 👤. Built **ahead of WO-4.5, which it unblocked.** The only row here on anything's critical path: it pulls WO-4.5's own real-data box from ~Sep 23 to ~Sep 16, and what it actually buys is **capacity rather than schedule** — an M of signal work moves off a teaching-week weekend into the last days of full capacity. *(Corrected 2026-08-27: this row read "and takes ten days off WO-G3" and that does not hold — see the note under § Ship 3's table.)* |
...
| 58 | [WO-1.52](phase-1-shell-store-roster.md#wo-152--a-planning-document-can-contradict-the-tracker-and-nothing-looks) A planning document can contradict the tracker and nothing looks | S | — | ✅ **2026-09-24** — all five Acceptance lines, no 👤 and no 📆, on the **second** verifier pass. `wo-sweep.mjs` § 26 reads the status claims in the planning documents it names by path — `IGNORE_DIRS` and `--audit` untouched — checks each against the tracker, and reports a contradiction in a live voice as a `REVIEW`; a `class="was"` passage, a dated sentence or a named dated document is excused. **The first pass failed**: a full stop in a dependency strip's description cell stood between the id and its state, so `plans/wo-3-18-runbook.html` yielded no claims and two stale strips went unread, and the "at least one claim" check counted all documents together. One correction round added a cell reader and made the count per document. **It paid for itself on its first run** — four stale strips in `plans/wo-3-18-runbook.html` and `plans/wo-7-1-runbook.html`, WO-7.1 and WO-8.12 still drawn unfinished — repaired in the commit after the tick. One line of the new cell reader (`tools/wo-sweep.mjs:3298`) survives its mutation, because no document exercises it |
| 79 | [WO-1.54](phase-1-shell-store-roster.md#wo-154--the-line-that-keeps-two-strips-apart-is-proved-by-nothing) The line that keeps two strips apart is proved by nothing | XS | — | 🎒 `tools/wo-sweep.mjs` — **Nothing blocks it.** An hour, whenever § 26 or the sweep is open for something else. Booked **2026-09-24** out of WO-1.52's verdict: the cell reader's "an id cell ends the search" line survives its own deletion, because no document § 26 reads puts two ids in one strip. Proved on a scratch file through `--claims-in`, never by planting the shape in a planning document |
...
| 18 | [WO-6.5](phase-6-calendar-glance.md#wo-65--a-tapped-day-opens-on-that-day) A tapped day opens on that day | S | — | ✅ **2026-09-23** — all six Acceptance lines, including the 👤, which the owner read on hardware in both orientations the same day. The date reaches the register as an argument (`resetRegistry(date)`), is held only in view state, and the register draws once on that day, with no flash of today. *(This cell carried 🎒 until it landed; it rode along with nothing in the end and was dispatched on its own.)* |
...
| 21 | [WO-5.2](phase-5-outreach.md#wo-52--templates) **Templates** | M | — | ✅ **2026-08-28** — all four Acceptance lines closed, no 👤 and no 📆. The three `Open` lines were answered by the owner before dispatch and are recorded in place: a **fourth header icon** opening a full view, a **permanent** block strip, and **eight starters that ship filled in and are never auto-loaded**. **One reading was waived and stays open** — the palette under a phone keyboard, which no headless run can close. **Row 22 is unblocked** |
| 22 | [WO-5.3](phase-5-outreach.md#wo-53--send-flow) **Send flow** | M | — | ✅ **2026-08-29** — **all seven Acceptance lines**, the last two closed by the owner on hardware: a draft opening in the default mail client on desktop and on iPad, and a copy landing in a sent folder. Both end outside the app, and the harness closes everything up to that edge — the `mailto:` URL is read off the handoff link and the body decoded back out of it matches the box character for character. `mailto:` only, as the architecture table requires. **Row 23 landed the same day.** *(This cell read "🔨 **2026-08-28** — five of seven Acceptance lines closed and both open ones are 👤" until 2026-08-30. Row 7's defect again, in its more expensive direction: it named as owed two readings the owner had already taken, and a row that says a work order is waiting invites somebody to go and pay a debt that does not exist.)* |
| 23 | [WO-5.4](phase-5-outreach.md#wo-54--contact-log--history) Contact log & history | S | — | **✅ DONE 2026-08-29**, all five Acceptance lines, no 👤 and no 📆. *The reason to build it was that nothing wrote a `contact` entry and two readers already read one — `src/log.js`'s cooldown and the quiet middle's "nor contacted all term" — so on Sep 2 the cooldown could not fire and WO-4.5's own 📆 fortnight would have measured a rule with no input. **That is now closed**: the handoff appends, and suppression is proved to follow from it off a student who trips two rules.* The stale line in `src/merge-fields.js`'s header went with it. **Read its note in `TESTING.md` before trusting the row**: the dispatch was killed by a session limit mid-mutation-proof and left a live `ruleId: ''` aimed straight at the second line — the cooldown dead in silence, every screen looking exactly as it does when it works. Found by `grep -rn MUTATION`, reverted, and the check that carries that line was red at the moment of death and green after |
| 59 | [WO-6.7](phase-6-calendar-glance.md#wo-67--the-glance-pages-stack-its-readers-and-the-quiet-day) The glance page's stack, its readers, and the quiet day | M | — | **First of three, and placed above row 24 rather than at the foot because it is row 24's first third.** Cut out of WO-6.4 on **2026-09-15**, owner-directed, for the window and not the work: one L is roughly a whole five-hour window on WO-1.39's figures, and a pause inside a dispatch cannot protect against the limit that kills the turn the pause needs — so the seams are work orders, at the stops the pipeline already has. Builds the stack, one reader per source in `src/glance.js`, and the quiet panel; rows 60 and 24 draw panels over its arrays. **Reads WO-4.5's post-cooldown hits as a count only**, which is the one thing here that touches the caution row 24 used to carry, and that caution expires ~Sep 16 either way. One `Open` line — the "not yet" sentence on the quiet panel — is the owner's before dispatch |
| 60 | [WO-6.8](phase-6-calendar-glance.md#wo-68--today-and-this-week-waiting-to-be-graded-and-closing-in) Today and this week, Waiting to be graded, and Closing in | M | — | **The day row 59 lands.** The three panels whose sources a live term has already confirmed — calendar items, the ungraded count, lead-time deadlines — none of which draws a signal. Carries WO-6.1's `**Owes**` pointer, moved with the grades-due box at the cut. Two `Open` lines for the owner before dispatch: whether a grades-due date also appears under *Today and this week* (proposed: no, deadlines live under *Closing in*), and panel 2's free-text titles staying up under a projector, recorded as a known edge rather than fixed |
| 24 | [WO-6.4](phase-6-calendar-glance.md#wo-64--the-glance-page) The glance page | M | — | **After rows 59 and 60, and not before ~Sep 16 — which is now one panel's wait rather than the page's.** *(This cell read "**Not before ~Sep 16, though its gates clear today** … the glance page draws Phase 4's signals, and drawing them before a real term has confirmed they fire correctly is building on a promise" until 2026-09-15, when the row was cut in three along exactly that line and dropped **L → M**.)* What is left here is panel 4 — the one that draws Phase 4 — and the lines about the page as a whole: the five-section walk, the two 👤 readings, the `supports` line, and a new line for the panel shutting under a projector, which the 2026-09-15 redraw found the page had never been asked about. It still depends on WO-4.5, whose open box is 📆 — the mark reports it code-complete and the tool lets it through, and that is still the mark working as designed and not a recommendation. One `Open` line — how many rows before `and N more ›` (proposed: four) — was the owner's number before dispatch, **answered four at dispatch, 2026-09-16** |
| 61 | [WO-4.6](phase-4-signals.md#wo-46--a-rule-that-cannot-fire-yet-is-a-different-sentence-from-a-rule-that-is-not-built) A rule that cannot fire *yet* | S | — | **After row 24, and the dependency is an argument rather than a build.** Booked 2026-09-15 out of WO-6.7's verdict. `inertRules()` answers *not built* and has answered `[]` since WO-4.4; nothing answers *not enough term yet*, so a teacher reading a thin concern column in week one cannot tell the two apart — WO-4.2's own confusion arriving from the calendar's side. **WO-6.7 declined the sentence and was right to**: the screen was never the missing piece, the engine answer was, and this row books it where the rules live. It changes no screen by construction, so it does not depend on row 24 to build — it is placed after it because *which surface wears this, if any* is a room argument best made with the glance page in front of you. A candidate for 🎒 if a `src/signals.js` sitting comes up first; the mark is the owner's |
| 62 | [WO-6.9](phase-6-calendar-glance.md#wo-69--the-review-count-opens-a-page-that-shows-the-review) The review count opens a page that shows the review | S | — | **After row 60, and nothing waits on it.** Booked 2026-09-16 out of WO-6.8's verdict. *1 review coming up* opens the calendar on this month, and `leadWindowOf()`'s window can end in the next, so a review early next month is counted on the glance page and missing from the page the tap opens. Nothing is disclosed and nothing is lost; the tap lands one page short. One `Open` line, where the tap lands, is the owner's before dispatch, and the Traps line refuses the option that puts a review's date on the button |
...
| 26 | [WO-7.3](phase-7-sync.md#wo-73--verification-complete) Verification complete | S | — | **Google's queue, then one function.** Depends on rows 9 and 20. Its code half is a single edit to `hostAllowsSignIn()`, paired with the console origins — already registered, so this side is the one behind |
| 27 | [WO-8.1](phase-8-packaging.md#wo-81--testingmd-complete-and-passing) `TESTING.md` complete and passing | M | — | **After Phase 7.** Reported gates-clear until 2026-08-28 on a `**Depends on**` field holding prose and no id — see its own note. Now gated on row 26, which is where it always belonged |
| 28 | [WO-8.2](phase-8-packaging.md#wo-82--demo-build) Demo build | M | — | After row 27 |
| 29 | [WO-8.3](phase-8-packaging.md#wo-83--accessibility-pass) Accessibility pass | M | — | After row 28 |
...
| 81 | [WO-8.16](phase-8-packaging.md#wo-816--a-first-time-visitor-meets-the-front-page-not-an-empty-gradebook) A first-time visitor meets the front page, not an empty gradebook | M | — | **After row 80, and before row 31, which now depends on it.** Booked **2026-09-25** beside row 80: that row fixes the URL Google is given, and this one fixes the bare domain a colleague is given. **A front door, not a gate** — the ruling it starts from, argued in its own Why. Two `Open` lines for the owner before dispatch: how firm to be on iPad Safari, and whether *use it in this browser* is remembered |
...
| 36 | [WO-1.35](phase-1-shell-store-roster.md#wo-135--a-ride-along-row-rises-to-the-top-when-the-rows-above-it-clear) A ride-along row rises to the top | M | — | ✅ **2026-08-28** — all seven Acceptance lines, no 👤 and no 📆, and **its own verifier booked row 37** on the keying its fixtures cannot tell apart. Built **before the next time `next` was trusted without reading the row it names.** Row 10 — re-placed to row 42 on 2026-08-29 — argues at length that it is last on purpose and must not lead; **its only fence is its position in this table**, and on 2026-08-28 rows 7, 8 and 9 cleared and it led anyway — silently, with nothing in the report saying the row it named had spent a paragraph asking not to be. **A fifth ordering constraint found outside the header fields, and the first that is not a dependency**: nothing blocks WO-8.13 and it blocks nothing, so no reading of `Depends on` would ever have caught it. Gives the `Suggested` column's *"rides along"* a parseable half, and makes `--audit` say when the shelf above a ride-along has emptied |
| 37 | [WO-1.36](phase-1-shell-store-roster.md#wo-136--two-fixtures-in-one-table-cannot-prove-a-per-section-shelf) Two fixtures in one table cannot prove a per-section shelf | S | — | 🎒 `tools/wo-gate.mjs` — **Rides along with the next sitting in the gate tool.** Booked by WO-1.35's verifier, which proved by mutation that collapsing `openAbove` to one global bucket leaves `--self-check` at 27 of 27: both 🎒 fixture rows sit adjacent in the **same** table, so the per-section keying the comment claims has no plant behind it. **No live symptom to work from, and less of one since 2026-08-29** — WO-8.13 was the first `⬜` in its section *and* in the document, so both readings agreed by printing the same NOTE; re-placing it to row 42 leaves no 🎒 row first anywhere, so they now agree by printing nothing. Neither state tells the two keyings apart. Rule the keying first; the two disagree about a sentence `--audit` already prints **It gained a second instance on 2026-08-30**, folded in rather than given a row: `applyRelease()`'s missing-result-file arm at `tools/wo-gate.mjs:1137` is the same shape — two behaviours behind one check, no fixture separating them — reported by WO-1.38's verifier and asked for by this work order's fifth Acceptance line |
| 38 | [WO-5.6](phase-5-outreach.md#wo-56--a-draft-survives-a-change-of-mind) **A draft survives a change of mind** | S | — | ✅ **2026-08-29** — all five boxes, no 👤 and no 📆, built the same day it was booked. **The first of the four the owner's 2026-08-29 sitting produced, and the one that stopped the app throwing away something a teacher typed.** The test is a **comparison against what the resolver produced**, not a keystroke flag, so a character typed and removed again rebuilds in silence and a rewritten subject over an untouched body still asks. The three doors are now `set…`/`apply…` pairs — nothing moves until she says so, and *cancel* is the absence of a call rather than an undo. **No stylesheet was opened**: every control in the panel is one `src/shell.css` already owns. **Row 41 extends it rather than re-deciding it** |
| 39 | [WO-5.5](phase-5-outreach.md#wo-55--the-two-sentences-the-flow-does-not-say) The two sentences the flow does not say | S | — | ✅ **2026-08-29** — all four Acceptance lines, no 👤 and no 📆. Two places the app knows something the teacher does not: a starter template is inert until saved and nothing says so, and a blocked draft says *"cannot be sent"* without saying what to do about it. Copy and one shared heading — the strip belongs to two screens over one `src/shell.css` section, so it changes in one place. **Its own verifier booked row 43** rather than closing it inside this row: making the block-strip head conditional left the surviving arm asserted nowhere |
...
| 68 | [WO-5.11](phase-5-outreach.md#wo-511--a-web-mail-handler-takes-the-pwa-window-with-it) A web mail handler takes the PWA window with it | XS | — | **Straight after row 40, and scheduled rather than 🎒** — the owner's call, 2026-09-12, because it is the thing hit every time a draft is sent from the laptop. `#outreachOpen` has no `target`, so with Gmail registered as Chrome's `mailto:` handler the installed PWA navigates to Gmail's bare compose page and loses the app. One attribute, `target="_blank" rel="noopener"`; an OS handler ignores it. **The iPad reading decides it** — Safari has left blank tabs behind for `_blank` on a `mailto:` before, and if it does here the attribute comes out. Nothing here detects the handler or explains a settings page. **🚫 STRUCK 2026-09-12, the same evening, and it was the laptop that decided it**: the iPad passed both ways, and on the laptop `_blank` opened a tab that sat blank on the `mailto:` URL and never reached the Gmail handler — installed app and plain Chrome tab alike. The attribute came out; the header note and an inverted harness check stay, so the next hand reaching for `_blank` gets a red line naming the reading. **WO-5.12 is the answer**, booked the same night: stop using the scheme for webmail at all. *(**Labelled `40a` until 2026-09-20**, and renumbered with row 67 for the same reason — `runningOrder()` never matched it. **Here it cost nothing and that is luck, not safety**: a 🚫 STRUCK row falls through `next()`'s loop unread anyway, so the broken label and the correct one produce identical output today. It is fixed because the* pattern *is what bites — `41a` one row below was the same keystroke and did real damage — and because a struck row that is ever un-struck would go invisible on the day it mattered.)* |
| 64 | [WO-5.13](phase-5-outreach.md#wo-513--every-template-whatever-the-recipient) Every template, whatever the recipient | S | — | ✅ **2026-09-20** — both Acceptance lines, no 👤 and no 📆, landed the day it was cut and before row 41, as required. *(Marked here 2026-09-20 during WO-5.8's maintenance: the phase file had read ✅ since the landing while this cell still read as booked, which is a table saying a done row's prerequisite is not.)* **After row 38, and it must land before row 41.** Cut out of WO-5.8 on 2026-09-20 with the reversal it carried: WO-5.3's seventh Acceptance line — proved and mutation-tested — filters templates on tone AND audience, and this drops the audience half. The tone half stays. **It is independent of multi-select and the ordering is not a preference** — a filter that needs one audience cannot survive a draft that has no single one, so reversing it after the picker exists means building the picker against a rule already known to be wrong. It amends four documents in place: WO-5.2's first Acceptance line, WO-5.3's seventh, both `TESTING.md` twins and `docs/data-model.md`. Four call sites, all in `src/outreach-view.js`; `templatesFor()` keeps its signature |
| 65 | [WO-5.14](phase-5-outreach.md#wo-514--the-compose-doors-take-a-list-and-which-header-the-others-ride-in) The compose doors take a list, and which header the others ride in | M | — | ✅ **2026-09-20** — all three Acceptance lines, no 👤 and no 📆, landed the day it was cut and before row 41, which consumed its Cc ruling without re-opening it. *(Marked here 2026-09-20 during WO-5.8's maintenance, for the reason given on row 64.)* **After row 38, before row 41, and independent of row 64 — either order.** Cut out of WO-5.8 on 2026-09-20, carrying **the argument that row said it owed**: which header the non-primary recipients ride in — To, Cc or Bcc — a disclosure question, not a formatting one. It is made here so the picker consumes a settled ruling instead of making one at the point in a build where the budget is thinnest. `encodeAddress()` is written for exactly one address and all three doors share it; a comma-joined list comes back with `%2C` between addresses, which is not the RFC 6068 grammar. **It lands invisibly** — the model passes one-element lists — so the multi-address behaviour is proved on hand-built drafts, which is clean because § 24 already forbids `src/outreach.js` a store import. **Names WO-5.3 and not WO-5.12 on purpose**: WO-5.12 built two of the three doors and is 🔨 on an Outlook reading the owner has no account for, and a dependency that cannot be discharged is a row that never starts |
| 41 | [WO-5.8](phase-5-outreach.md#wo-58--several-recipients-and-one-of-them-is-primary) **Several recipients, and one of them is primary** | M | — | ✅ **2026-09-20** — all five Acceptance lines, no 👤 and no 📆, built and verified the same day the cut that made it an M was written. Its one open judgment went the other way from WO-5.3's: an addressless recipient is **genuinely unchoosable**, and the standing check at `tools/verify/outreach.mjs:873` was rewritten rather than re-asserted. **After rows 64 and 65.** The picker itself: several recipients at once, exactly one primary, the primary being who merge fields resolve against and who the message is addressed to. `recipientKey` is one string today and every consumer downstream assumes one. **Cut in four on 2026-09-20 — this row keeps its id, this slot and the picker**, which is what keeps row 42's 🎒 shelf resolving, since that cell names row 41 as the sitting that opens `index.html`. *(This cell read "**After row 38, and it reverses a verified Acceptance line.** WO-5.3's seventh — proved and mutation-tested — filters templates on tone AND audience; this drops the audience half, because a message to both guardians and the counselor has no single audience. The tone half stays. Whoever builds it amends that line in place. It also owes an argument nobody has made yet: which header the non-primary recipients ride in is a disclosure question, not a formatting one" until 2026-09-20, when the reversal went to row 64 and the owed argument to row 65.)* One trap the Acceptance line hides: an addressless recipient is **choosable and then blocked** today, not unchoosable, and the harness clicks that chip to assert the block. *(**Answered on the landing, 2026-09-20: unchoosable.** That sentence is the state this row inherited, not the state it left — the chip is refused at the door on a `aria-disabled` live button, the `kind: 'recipient'` reason did not go vacuous, and a new check reaches it through a student with nobody addressable.)* |
| 67 | [WO-5.12](phase-5-outreach.md#wo-512--a-webmail-door-that-is-not-a-mailto) A webmail door that is not a `mailto:` | S | — | **After row 41, booked 2026-09-12 the night WO-5.11 was struck, and it is the answer to what struck it.** `mailto:` cannot reach a teacher whose mail is Gmail in a tab — the same-window link takes the app and a `_blank` one reaches nothing — so this stops using the scheme for webmail: an https compose URL for Gmail or Outlook on the web behind a per-device preference, `mailto:` still the default, the iPad unchanged. **It revisits `privacy.html` and `docs/FERPA.md` before any code is written** — the shared data-flow statement says the draft goes to *your own mail app*, and with this built it goes into the URL of a Google page; WO-8.12 is a dependency for that reason. The 👤 line that decides it is the one WO-5.11 could not close. *(**This row was labelled `41a` from its booking on 2026-09-12 until 2026-09-20, and for those eight days `next` could not see it at all.** `runningOrder()` matches `/^\|\s*(\d+)\s*\|\s*\[(WO-[\dG][\w.]*)\]/` — `41a` is not `\d+`, so the row was skipped before its id was ever read, and `next()` does `if (!wo) continue;` without a word. **That is the WO-1.24 failure this directory exists to prevent**, and it cost exactly what that one cost: WO-5.12 has been 🔨 with one open 👤 line since 2026-09-14, `next` names every 🔨 row it steps over as* part-built work, pick it up where it stopped*, and it has never once named this one. Found on 2026-09-20 while cutting WO-5.8, by a check that listed the rows the regex matches and noticed which id was missing. **The label is `67` because the number is a label and never a position** — `41a` was someone reaching for a positional one, which is the exact reading § "the running order" warns against three times; the row has not moved, and the cell above still names row 41 correctly.)* |
...
| 66 | [WO-5.15](phase-5-outreach.md#wo-515--one-contact-several-audiences) One contact, several audiences | S | — | **After row 41, which is the only row here that could not be started before it.** Cut out of WO-5.8 on 2026-09-20 and **the one of the four that was not on its Deliverables list** — `writeContact({ audience })` records a scalar enum, pinned in `docs/data-model.md` and read by three: the history chip, `lastContactAbout()` (the cooldown), and signals-view's *"You wrote to their guardian about this on Sep 6"*. A draft to two guardians **and** the counselor has no single value for it. **WO-5.8 lands writing `audienceOf(primary)`, which is incomplete rather than false** — the message is addressed to the primary — so this is a real row and not a defect left open. `src/log.js` has no updater and no delete, so whatever is decided is decided on the write and no entry already in a teacher's document is ever rewritten. **Found by reading the row, not by building it**, which is the argument for cutting an L as its own sitting: a cut is also an audit |
...
| 44 | [WO-5.9](phase-5-outreach.md#wo-59--the-hitless-draft-is-written-but-never-driven) The hitless draft is written but never driven | S | — | ✅ **2026-08-30** — all four Acceptance lines, no 👤 and no 📆, and **it rode along exactly as booked**: one sitting inside `tools/verify/contact-log.mjs`, two checks, one more student, and no app code at all. `recordHandoff()`’s false branch is driven for the first time — a draft for a student **no rule has fired for**, opened through the student record’s door because there is no card for her — and the record it writes is handed straight back to `lastContactAbout()`, which is the loop `cooldown-quiet.mjs` structurally cannot close. **The fixture takes a THIRD student rather than Ben**, the choice the Traps line left open: Ben’s empty history is what a projected one is compared against character for character, so a contact written for him reddens that check for a reason that looks unrelated. Both regressions proved by real runs and reverted by hand — dropping the guard reddens **both** new checks while the anchor keeps its `href` (the mail app opens and nothing is logged), and inventing `'manual'` is caught by the READER conjunct rather than by the string comparison |
| 45 | [WO-1.39](phase-1-shell-store-roster.md#wo-139--the-window-is-spent-by-measure-and-read-by-feel) The window is spent by measure and read by feel | M | — | *(**This row was numbered 46 from 2026-08-30 until later that day**, and the gap it left is the only one in this table that never held a row. Its sitting booked two rows and appended one: WO-1.38 filled the vacancy at row 10 that WO-8.13 left when it moved to 42, so the count owed the foot was 44 + 1 and 44 + 2 was written. **Renumbering it cost nothing, which is why it was done here and is refused elsewhere** — no prose in this file cites row 45 or 46, and this is the last row, so nothing below it moved. Read the WO-1.37 note above before assuming that generalises: a splice above an occupied row costs every reference that names a number, and `runningOrder()` ignores the digits either way.)* After WO-1.38. `--start` asks whether the gates are clear and nothing asks whether there is a **window** to fit the dispatch in. **The `Size` column is not a capacity estimate**: S ran 6.8–13.3M weighted units and M ran 7.2–15.7M, overlapping almost entirely, and the priciest S outran five of six Ms — so a budget rule that fires on the big ones fires on the wrong ones. The data is already on disk at a **two-second lag** in the transcripts `wo-cost.mjs` reads. **Advisory, never refusing**, on row 36's precedent that only a person can pick. Three traps carry it: a project-scoped read under-reads (16 project dirs, 3 active in 24h), an account swap over-reads with nothing on disk recording it, and a `0.0M` meaning *no data* must not look like one meaning *fresh window* |
| 46 | [WO-1.40](phase-1-shell-store-roster.md#wo-140--the-file-a-human-types-is-outside-every-fence) The file a human types is outside every fence | S | — | ✅ **2026-08-30** — booked that morning by WO-1.38's verifier as a thing nobody had claimed, and built the same day; it was the fifth of the tracker rows — 12, 14, 15, 16, 46. WO-1.38 changed the pipeline in **six** files and wrote five: `.claude/commands/wo.md` went on telling its caller to relay an Acceptance list from the one stop that now deliberately returns none, and the only material for satisfying it is the implementer's self-claims — **the WO-3.5 defect one level above the orchestrator, in the file with no agent between it and a human**. *The prose was corrected the same day, directly; this row was the fence, not the repair.* **`.claude/commands/` was named nowhere in this system** — no work order, no `--audit`, and `.claude` sits in `wo-sweep.mjs`'s `IGNORE_DIRS`, which is right for app code and was not a reason for no check at all. **§ "The pipeline's own files" above names it now**, and § 21 reaches both halves of the pair by path rather than by widening that walk. **Its second trap is what the section became:** the two files are *supposed* to differ, so what is wanted is contradiction rather than symmetry, and `REVIEW` is the honest state |
| 47 | [WO-1.41](phase-1-shell-store-roster.md#wo-141--the-two-files-that-must-never-drift-apart-are-held-together-by-nothing) The two files that must never drift apart are held together by nothing | M | — | ✅ **2026-08-30** — booked alongside row 46 that morning and built the same day; it was the sixth of the tracker rows — 12, 14, 15, 16, 46, 47. It was **deliberately booked before row 46 was built**: `CLAUDE.md` stated *“a rule changed here is changed there in the same sitting”* in bold and **nothing enforced it** — a grep over `tools/*.mjs` found two mentions of `AGENTS.md`, neither a check. It was row 46's defect on a surface an order of magnitude larger: 207 lines against 627, with the shared rules deliberately worded differently in each. **It depended on row 46 rather than duplicating it** — these two files sit at the repository root and the sweep already walked them, so the half that carried over was the comparison, and booking this first is what made row 46 build one that takes a pair; the change here is **+88 lines and every one of them inside `DRIFT_PAIRS`**, with the comparison engine below byte-untouched. **Its fourth trap became the shape of the thing:** the set of shared rules cannot be inferred from prose, so the four claims are hand-written at `tools/wo-sweep.mjs:2418-2505`, and that list is **the fourth thing that has to be kept in step** — after `CLAUDE.md`, `AGENTS.md` and the map row in § “The pipeline's own files” above. **What it cannot see is itself:** the two sentences this work order made false — `AGENTS.md`'s *“nothing enforces that sentence yet”* and `CLAUDE.md`'s *“not yet a watched pair”* — are structurally invisible to the fence it built, one an italic parenthetical the engine skips and the other in the **reference** half, which is searched for anchors and never scanned for contradiction |
| 48 | [WO-1.42](phase-1-shell-store-roster.md#wo-142--the-sweeps-own-check-count-is-maintained-by-hand) The sweep's own check count is maintained by hand | S | — | **Whenever the tracker is quiet, and it is the seventh of the tracker rows** — 12, 14, 15, 16, 46, 47, 48. Booked **2026-08-30** on WO-1.40's verifier's proposal. `tools/README.md` records how many checks `wo-sweep.mjs` runs and **a person types that number**; WO-1.40's verification found it reading **33 against a tool that ran 34**, already stale before that work order touched it. **This is not cosmetic** — a stale count in `tools/README.md` is the tell that betrayed the dead dispatches on WO-3.26 and WO-4.4, and `plans/dispatch-retro.md` calls it the cheapest single thing to look at; a tell that drifts on its own has quietly stopped reporting while the recovery procedure still points at it. It is § 11's census turned on the sweep itself. **Read its first trap before writing a line:** assert against `results.length` at runtime and never a second hard-coded number, or the check is one more hand-maintained figure policing a hand-maintained figure |
| 49 | [WO-1.43](phase-1-shell-store-roster.md#wo-143--a-broken-excuse-reads-as-a-contradiction-and-a-vanished-claim-reads-as-green) A broken excuse reads as a contradiction, and a vanished claim reads as green | S | — | ✅ **2026-09-23** — all five Acceptance lines, no 👤 and no 📆, and **taken on its own rather than riding with row 48**, which is still ⬜; it was the eighth of the tracker rows — 12, 14, 15, 16, 46, 47, 48, 49. § 21 now names a lost `confinedTo` region on the REVIEW branch as well as the green one, and prints occurrences **per claim** (`#1 7, #2 0, #3 5, #4 5` where it used to print `17`) while a zero stays green, as its first trap required. Matching and excuse logic are unchanged; the one new line in the loop is the counter. **The count paid for itself on its first run:** pair 1's claim #1 (*relay the report in full*) has **0** occurrences in `.claude/commands/wo.md`, so it currently checks nothing — green by this row's rules, and whether to re-point or retire it is the owner's call, not yet made. Its verifier named two uncovered cases: two regions lost at once, and the `'none'` label, which nothing can reach today |
| 51 | [WO-1.45](phase-1-shell-store-roster.md#wo-145--a-green-run-cannot-say-whether-the-containment-is-still-there) A green run cannot say whether the containment is still there | S | — | 🎒 WO-1.43 — **an hour to fold into whichever sitting opens `wo-sweep.mjs` next**, and row 49 is the one already booked to it. Booked **2026-08-31**, owner-directed, on WO-1.44's verifier's first proposal. Nothing blocks it; WO-1.44 ✅ is its only dependency. **It carries no date constraint** — it edits `wo-sweep.mjs`, the grep half, and cannot affect a harness run, so unlike row 52 it could be taken before Sep 2 without trading anything. **Why it exists:** restore the bare `for (const s of BROWSER_SECTIONS)` loop and the harness prints `1284 · 1284 · 0 · 0` with a green sweep — **a green run is what a healthy harness and an uncontained one both produce**, and today the only proof is a hand-planted mutation. Third instance of the WO-1.40/WO-1.42 shape in four days. **Read its first trap before writing a line:** a grep for `runSection(` is a tripwire and not a proof, and the failure mode of this row is shipping the weak version described as the strong one |
| 52 | [WO-1.46](phase-1-shell-store-roster.md#wo-146--four-fixtures-still-guess-a-date-and-one-of-them-is-the-twin-of-the-one-that-broke) Four fixtures still guess a date | S | — | **Not before 2026-09-02**, owner-directed — and the constraint is the point of the row's placement rather than a note on it. It edits four fixture files inside `tools/verify/`, the surface WO-1.44 just stabilised on five green weekdays, and re-opening that two days before the app meets students trades a real risk for one that has already been downgraded. Booked **2026-08-31** on WO-1.44's verifier's second proposal. **Why it exists:** WO-1.44 derived its date instead of guessing it and closed **two sites of six**; `register-opens-on-term.mjs:75`, `term-edges-marking.mjs`, `term-ended.mjs` and `today-goes-to-term.mjs` still guess, were green on all five weekdays driven, and **none is immune by construction** — which is the evidence that failed on 2026-08-30. **Why it can wait:** before WO-1.44 a collision killed the run and hid 766 checks; it now reddens its own section and lets the rest finish, so this **fails safely**. **Read its second paragraph before scheduling it:** one of the four is a twin — `nodeWeekdayAhead(9)`, the same construct on the same offset — and three are cousins planting term edges, so four sites are not four equal jobs |
| 63 | [WO-1.53](phase-1-shell-store-roster.md#wo-153--the-residue-meets-the-registers-earlier-page-for-a-week-every-september) The residue meets the register's earlier page for a week every September | S | — | **Pairs with row 52 — same directory, same collision, one sitting — and not the same fix.** Booked **2026-09-17** out of WO-6.9's verdict, the first row here booked from a verifier's *real-clock* run rather than its shifted one: same tree, `--today=2026-09-16` green at 1411, the real clock red at 1396 · 1391 · 5. **Why it exists:** `attendance-passes.mjs:2172` asserts the register's earlier page is empty before measuring across it, and from **2026-09-17 through 2026-09-23** that page holds the `2026-09-09` record `classes-terms.mjs:705` plants on purpose, and on the 24th the residue is the dropped edge and one check goes red on its own — eight days, driven both ends with `--today` the day it was booked. Not a site row 52's helper fits — that helper walks *forward* to a date the fixture is free to pick, and the earlier page is whatever the app draws. **Why it can wait, and why not long:** it fails safely, its own section and fifteen named losses, so nothing about the Sep 2 build changes — but **every real-clock `verify-shell` between the 17th and the 24th is red**, `--today` outside the window is the pre-deploy check until it lands, and after the 24th only the flag can show it. Read its Traps before choosing a side: two of its three shapes are wrong, and the edge on the 24th fails quietly enough to be left unfixed |
| 53 | [WO-1.49](phase-1-shell-store-roster.md#wo-149--a-comment-counts-three-document-level-listeners-and-there-are-twelve) A comment counts three document-level listeners and there are twelve | S | — | 🎒 `src/shell.js` — **an hour to fold into whichever sitting opens `src/shell.js` next**, and **[WO-1.48](phase-1-shell-store-roster.md#wo-148--a-date-field-cannot-tell-mid-typing-from-cleared-and-the-app-infers-it-anyway) is the shelf it is waiting for** — it opens all five date fields and the census paragraph this row is about sits directly above them. **That shelf is named by ID and not by a row number, and the reason this cell first gave for that was false when it was written** — it said WO-1.47 and WO-1.48 *"have no row in any running-order table"* and that `next` "has never been able to reach either", when **the very commit that added this row added theirs**: rows 54 and 55, `2c51015`, 2026-09-06. Repaired 2026-09-06. Naming the shelf by ID stands anyway, for row 50's reason rather than this one — **a row's `#` is a label and never its position**, so a shelf named "row 55" would go stale the next time anything is inserted above it. Nothing blocks it. Booked **2026-09-06**, owner-directed, on WO-1.47's implementer's own proposal — it found the defect mid-correction-round, declined to widen a work order that had just failed for a stale comment, and said it would rather the count were booked than smuggled. **Why it exists:** the delegation preamble says *"Three other document-level listeners live further down"* and there are **eleven** below the `click` it excludes — twelve `document.addEventListener` calls in the file. **It was already wrong before WO-1.47** (`beforeinput`, `change`, `focusin` and the three drag listeners all postdate the sentence) and WO-1.47's `focusout` made it one worse, with nothing in the repository noticing across any of them — the WO-1.40/WO-1.42/WO-1.45 shape for the fourth time, a hand-typed number in prose that nothing checks. **Read its ruling before writing a line:** the answer is not "type 11" — take the number out and close the class, or repair it *and* fence it at runtime against the file the way WO-1.42's is against `results.length`. **Writing a fresh number and stopping is this row's own defect re-armed with a later expiry date** |
| 54 | [WO-1.47](phase-1-shell-store-roster.md#wo-147--a-zero-typed-into-a-date-field-clears-the-date-and-takes-the-field-with-it) A zero typed into a date field clears the date and takes the field with it | S | — | ✅ **2026-09-06** — built and verified 2026-09-03, the 👤 iPad line closed 2026-09-06. **Booked and dispatched by ID on 2026-09-03 with no row here**, which is why this one is being written three days late: `next` could not reach it, and nothing reported that. Typing `0` as the first digit of a date fired `change` on Chromium's empty read, and `assignmentDateCommitted()`'s unconditional `wrap.replaceWith(dateField(...))` **threw away the element under the caret** — focus to `BODY`, every later digit nowhere, and an assignment left with no due date at all, silently. The rebuild moved from `change` to `focusout` at all five sites, where it cannot take a caret by construction. **It is a guard and not the repair**, and what it gives up is named rather than hidden — row 55 is the repair. **It failed verification on its first pass**, on its own line 4: four census entries in `src/shell.js` still said the rebuild ran on `change`, and the stated check missed them because it grepped **function names** while those comments name only the **event**. The correction round replaced it with a grep on the event across a `-B3 -A3` window and proved it against the pre-repair tree — 29 hits, each read. **Read that before writing a comment-accuracy check anywhere:** the check that fails is the one aimed at the wrong token |
| 55 | [WO-1.48](phase-1-shell-store-roster.md#wo-148--a-date-field-cannot-tell-mid-typing-from-cleared-and-the-app-infers-it-anyway) A date field cannot tell mid-typing from cleared, and the app infers it anyway | M | — | **After a teaching week, and deliberately at the foot** — it is the real repair for row 54's defect and it is still not a thing to cut into one. New design across five surfaces, a control this app has never had, and it wants a real iPad rather than a harness. Booked **2026-09-03** alongside row 54 and likewise with no row here until 2026-09-06. Depends on WO-1.47 ✅. **Why it exists:** the guard stops the app eating the field a teacher is typing into; it does not remove the reason it could — a native date input reports `''` for both *mid-typing* and *deliberately emptied* and hands the page nothing to tell them apart, so every repair that keeps inferring *she cleared it* from an empty value is arguing with the ambiguity instead of removing it. An explicit **Clear** hangs the reset off a button where there is no caret, and the `focusout` rebuild comes back out. **It has a measured before-picture rather than an assumed one:** the case row 54 knowingly gave up was read on hardware 2026-09-06 and written down as failing in `TESTING.md` § WO-1.47 — cleared a due date holding September 4, stayed in the field, tapped September 4 again, field stayed empty. **There is nothing to lift:** Roll Call! has the same native fields, no clear affordance and no rebuild of any kind — which is also the evidence that never replacing the element is why it does not have row 54's bug |
| 56 | [WO-1.50](phase-1-shell-store-roster.md#wo-150--a-document-you-can-read-is-not-a-document-anything-checks) A document you can read is not a document anything checks | M | — | **At the foot, and the owner put it there** — booked **2026-09-06** out of the sitting that built `tools/data-viewer.html`, which is not a work order and has no row: it was owner-directed and landed as one commit. **Lower priority than everything above, in the owner's own framing**, and the row exists so the concept is not lost rather than so it gets scheduled. **Why it exists:** the viewer makes [`known-bugs.md`](../known-bugs.md) § 2 — a class reading *1 unconfirmed* with every student on the grid present — **readable**, and cannot make it **noticed**; nobody opens a viewer about a class that looks fine. **Read the Traps before the drawing:** the checks are the easy half, and the hazard is that a panel re-deriving `countsFor()` puts the defect in the tool while wearing a report's clothes — plus a third reader of the masking rule that already failed once, on the day the viewer was built. **Nothing blocks it**, and it blocks nothing. Not 🎒: nothing rides along with a new file |
| 57 | [WO-5.10](phase-5-outreach.md#wo-510--the-status-line-is-the-one-field-the-projector-does-not-empty) The status line is the one field the projector does not empty | XS | — | ✅ **2026-09-15** — built and handed off 2026-09-14, verified the same day with one FAIL on Acceptance line 2, and that line **reworded on the owner's ruling** 2026-09-15 rather than paid for with a hook in `src/shell.js`: the sentence redrawn when the projector goes off comes back beside everything else the same flip un-hides. *(Was 🎒 on `src/outreach-view.js`, with row 41 as its shelf; taken on its own instead, so the mark is off.)* Its booking: WO-5.8 rewrites the recipient handler that writes the first of the two sentences. Booked 2026-09-12 out of WO-5.7's verdict. `outreachModel()` puts `status` on `base` before it asks about the projector, so a rebuilt-for or handed-off sentence naming a guardian or a child survives the blocked branch inside the hidden form — `display: none`, which the flow's own rule says is not a redaction. Not a live disclosure today; the harness is green over it only because its fixture flips the projector before anything has written a status. One model line, one check that switches recipient first |
...
*Row 17's `Depends on` is WO-3.4 and WO-4.1, both ✅ — but* [WO-4.3](phase-4-signals.md#wo-43--praise-signals)
...
***Row 42 was booked 2026-08-21 as row 10, and it is last on purpose.*** [WO-8.13](phase-8-packaging.md#wo-813--the-about-modal-names-two-documents-and-not-the-licence)
*is one link in the About modal naming the licence* `LICENSE.md` *has carried since that morning.
Nothing blocks it and nothing depends on it, which is exactly why it belongs at the foot of the
table:* `next` *stops at the first* `⬜` *in document order, so a row placed higher would put an hour
of `index.html` in front of an M of sync work and five signal boxes that race the term.* **It is the
...
*(**It was re-placed on 2026-08-29, and the paragraph above is now an argument for where it sits
rather than a description of it.** That paragraph said "it sits at the foot of the table" from the
morning it was written, and the claim was true for one day: thirty-one rows landed above it over the
following week and* **not one of them moved it**, *so by 2026-08-28 the row that had spent a
paragraph asking not to lead was leading.* [WO-1.35](phase-1-shell-store-roster.md#wo-135--a-ride-along-row-rises-to-the-top-when-the-rows-above-it-clear)
*made that visible — the* 🎒 *mark, and an* `--audit` *NOTE when a marked row's shelf empties — and
the NOTE is what produced this move, the owner's call, in the sitting that landed WO-5.6.* **The
mark is what protects the row now; the position never did.** *It was re-placed because the argument
